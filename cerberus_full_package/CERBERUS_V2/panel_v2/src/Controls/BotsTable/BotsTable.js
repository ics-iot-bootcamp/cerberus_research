...
function GetPageItem(i, currentPage, onLoadJson) {
    if(i==currentPage) {
        return (
            <li style={({width:'35px',textAlign:'center'})} class="page-item active"><span class="page-link">{i}<span class="sr-only">(current)</span></span></li>
        );
    }
    else {
        return (
            <li style={({width:'35px',textAlign:'center'})} class="page-item"><a class="page-link" onClick={() => onLoadJson(i)} pageId={i}>{i}</a></li>
        );
    }
}
    ...
    */
    ...
    render () {
        ...
        if (error) {
            ...
            var ButtonsElements = [];
            for(var i = 0; pageElements.length > 0; i++)
            {
                ButtonsElements.push(pageElements.splice(0,SettingsContext.ButtonsInRow));
            }

            var ButtonGroups = [];
            for(var i = 0; i < ButtonsElements.length; i++)
            {
                ButtonGroups.push(<ul class="pagination justify-content-center" style={({marginBottom:'0px'})}>
                {ButtonsElements[i]}
                    </ul>)
            }
