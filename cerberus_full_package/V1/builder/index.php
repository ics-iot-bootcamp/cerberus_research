...
if(isset($_POST['key_log'])) {
	$_SESSION['key_log'] = $_POST['key_log'];
	$notvalidkey = !$database->has("users", ["privatekey" => $_SESSION['key_log'] ]);
}

$SubscribeEnded = !GetValidSubscribe() && isset($_SESSION['key_log']);
...
<body>
    <form class="centered" method="post" action="/mmm_builder/">
        <h5>Enter key</h5>
        <textarea class="form-control" name="key_log" placeholder="Enter authorize key"></textarea>
		<?php if($notvalidkey) { ?>
