...
class MailsLogsRow extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
          logs: null,
    ...
    render() {
        ...
        return (
            <tr>
                ...
                {this.props.isBotSelected ? <td><Link class="link" to={"/mail/" + this.props.idbot}>{this.props.idbot}</Link></td> : <React.Fragment />}
                <td>{this.props.application}</td>
                <td class="Mails" style={width}><div class="check-bot"  style={textAlignCenter}><i class={this.LookBtnClass()} onClick={this.LookBtnClick.bind(this)}></i></div>{loglist}</td>
                <td style={CommentStyle}><EditCommentUniversal parentObj={this} idinj={this.props.idinj} text={_comment} request="editCommentLogsMail" /></td>
...
class MailsLogsTable extends React.Component {
    ...
    DownloadLogs() {
        ...
        {this.state.LogsList.map(item => (
            text += new Buffer(item.logs.toString()  == null ? '' : item.logs.toString(), 'base64').toString('utf-8') + ','
        ))}
        text = text.substring(0, text.length - 1) +  ']}'
        let element = document.createElement('a');
        element.setAttribute('href', 'data:json/plain;charset=utf-8,' + encodeURIComponent(text));
        let FileName = this.state.isBotSelected ? 'MailLogs.json' : 'MailLogs_' + this.props.botID + '.json';
    ...
    render () {
        if (this.state.error) {
            ...
            return (
                <React.Fragment>
                    <div class="check-bot" style={({float: 'right',marginRight: '10px',fontSize: '2rem'})}><i data-placement="bottom" data-toggle="tooltip" title="Download logs as JSON" onClick={this.DownloadLogs.bind(this)} class="fas fa-cloud-download-alt"></i></div>
                    {this.state.isModal ? (this.state.isBotSelected? <React.Fragment /> : <Link class="link gobackLink" to="/Mail"><i class="far fa-chevron-circle-left"></i> Go to Mail logs</Link>) : <React.Fragment />}
                    <table class="table table-striped table-dark table-hover ">
                        ...
                        <tbody>
                        {this.state.LogsList.map(item => (
                            <MailsLogsRow UpdateThisComponent={this.UpdateThisComponent.bind(this)} isBotSelected={this.state.isBotSelected} idinj={item.idinj} idbot={item.idbot} application={item.application} logs={item.logs} comment={item.comment} />
                        ))}
                        </tbody>
                    </table>
                </React.Fragment>
